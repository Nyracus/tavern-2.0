...
export class AuthService {
  async register(data: RegisterInput) {
    const exists = await UserModel.findOne({
      $or: [{ _id: data.id }, { email: data.email }, { username: data.username }],
    }).lean();
    if (exists) throw new AppError(409, 'User with this id/email/username already exists');
  ...
  publicUser(u: Pick<IUser, '_id' | 'email' | 'username' | 'displayName' | 'avatarUrl' | 'role' | 'createdAt' | 'updatedAt'>) {
    return {
      id: u._id,
      email: u.email,
      username: u.username,
